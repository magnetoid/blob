...
from . import handles, janus_agent
...
async def found(
    ...
    *,
    ...
    # Seeded here rather than offered later, and in this transaction. A team is promised
    # an agentic workspace, and that promise is kept or broken in the first minute: either
    # the agent is already in #general when the founder arrives, or it is a setup task they
    # may never do. The boot reconcile alone cannot keep it — on a fresh deployment the
    # first workspace is founded at signup, *after* boot, so without this hook it would
    # hold no agent until somebody restarted the server. Silently a no-op when Janus is
    # not running beside us — there is nothing to seed, and a signup must not fail over
