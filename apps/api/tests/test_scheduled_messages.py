...
async def make_due(scheduled_id: str) -> None:
    ...
    async with SessionFactory() as session, session.begin():
        await session.execute(
            sql("UPDATE scheduled_messages SET send_at = now() - interval '1 minute'"
                " WHERE id = :id"),
            {"id": scheduled_id},
        )
...
class TestTheSweep:
    ...
    async def test_leaving_the_channel_stops_it(self, team: dict) -> None:
        ...
        assert made.status == 200, made.body
        assert (await team["member"].post(f"/api/channels/{channel_id}/leave", {})).status in (200, 204)
        await make_due(made.body["scheduled"]["id"])
    ...
    async def test_cancelling_someone_elses_is_not_possible(self, team: dict) -> None:
        ...
        answer = await team["member"].delete(
            f"/api/scheduled/{made['body']['scheduled']['id']}"
        )
