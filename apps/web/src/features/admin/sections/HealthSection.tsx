...
export function HealthSection({ onError }: { onError: (message: string | null) => void }) {
  const load = useCallback(() => api.admin.health(), []);
  const { data: health, loading } = useAdminData(load, [], onError, 'Health unavailable.');

  if (!health) {
    return <p className="muted">{loading ? 'Checking…' : 'Health unavailable.'}</p>;
  }
