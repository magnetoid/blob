...
- L197 `increment_mentions(session: AsyncSession, user_ids: list[str], channel_id: str)` (function) — Called by the notify worker for each recipient a message actually pings.
- L230 `broadcast(user_id: str, state: ReadStateOut)` (function) — Other devices belonging to this user need to clear their badge too.
- L235 `list_for_user(session: AsyncSession, user_id: str)` (function)
- L257 `total_mentions(session: AsyncSession, user_id: str)` (function) — Total badge across the workspace — what the tab title and favicon show.
