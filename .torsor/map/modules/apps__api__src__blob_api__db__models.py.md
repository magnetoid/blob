...
- L1377 `PluginCommand` (class) — A slash command an app provides.
- L1403 `PluginSecret` (class)
- L1413 `PluginGrant` (class)
- L1427 `AgentDelegation` (class) — Somebody the owner has let command their agent.
- L1474 `BotToken` (class)
- L1488 `PluginDelivery` (class) — The outbox. Written in the transaction that caused the event, drained by the worker.
- L1520 `FeedbackTicket` (class) — Added by 0007. A bug report, feature request or note, with its diagnostics.
- L1559 `McpToken` (class) — A person's own assistant, holding their permissions from outside the browser.
