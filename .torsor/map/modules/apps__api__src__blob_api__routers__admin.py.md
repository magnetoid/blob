...
- L833 `InstanceUser` (class)
- L845 `InstanceUsersOut` (class)
- L849 `InstanceWorkspace` (class)
- L859 `InstanceWorkspacesOut` (class)
- L864 `instance_users(_admin: SessionUser=Depends(require_instance_admin))` (function) — Every account on the server, whichever workspace it belongs to.
- L903 `instance_workspaces(_admin: SessionUser=Depends(require_instance_admin))` (function) — Every workspace on the server, with enough to tell them apart at a glance.
- L949 `CreateWorkspaceInput` (class)
- L953 `CreatedWorkspaceOut` (class)
- L960 `create_workspace(payload: CreateWorkspaceInput, request: Request, admin: SessionUser=Depends(require_instance_admin))` (function) — Make another workspace, owned by whoever made it.
