...
- L36 `Installed` (class)
- L44 `bot_email(slug: str)` (function)
- L48 `by_id(session: AsyncSession, plugin_id: str, workspace_id: str)` (function)
- L60 `granted_scopes(session: AsyncSession, plugin_id: str)` (function)
- L70 `install(session: AsyncSession, *, workspace_id: str, manifest: Manifest, installed_by: str, source_repo: str | None=None, source_ref: str | None=None, reserved_commands: frozenset[str]=frozenset())` (function)
- L150 `_create_bot_user(session: AsyncSession, workspace_id: str, plugin_id: str, manifest: Manifest)` (function) — A real user row, with no password so it can never sign in through the front door.
- L175 `_available_display_name(session: AsyncSession, workspace_id: str, wanted: str)` (function) — Mentions resolve by display name, and the unique index is partial on active users.
