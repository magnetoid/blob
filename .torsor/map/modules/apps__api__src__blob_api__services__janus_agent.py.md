...
- L171 `_repoint(session: AsyncSession, plugin_id: str)` (function) — Move a row installed earlier onto the configured address and secret.
- L202 `ensure_everywhere()` (function) — Reconcile every workspace at boot. Returns how many gained the agent.
