...
**Summary:** 30 commit(s); 418 files changed, 11552 insertions(+), 3213 deletions(-); ran 5× check_drift

Commits:
- 047baf4 Pull MinIO from a registry that still has it
- d317523 Put the claim beside the sign-in
- 05ca96b Let a citation look like what it points at
- aa730e2 Mark a summary a model wrote as one
- 64055f2 Give agents a colour of their own
- 3f15c94 Drop the last two settings of a thing we never used
- 7d44025 Cover the room the question is actually asked in
- 5f5d9cc Let the agent say who is in the room
- e5b23e0 Stamp the history What's new shows
- bf8fabd Give the consoles one shape, and the admin pages one home each
- 709ae62 Let @Blob read the workspace, as the person who asked
- b87288f Let the model layer run tools, so an agent can act
- b2ebb53 Stop an unconfigured media server from keeping chat down
- 5e14856 Make meetups reachable and give them their channel's access
- c30b3ec Quote LiveKit's key pair, and stop generating a domain for it
- 1e11f55 Run LiveKit beside Blob, so meetups have somewhere to connect
- ad6c274 Merge main into meetups, keeping main's shell
- dea137b Bring CLAUDE.md up to date with the agentic surface
- 7fc0762 Meetups: live audio and video rooms, on LiveKit
- 623ac4d feat: later bookmark, person cards, and the files tab (#13)
