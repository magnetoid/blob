...
**Summary:** 64 commit(s); 441 files changed, 50685 insertions(+), 4129 deletions(-); ran 37× check_drift

Commits:
- 0ca55e1 Record the stale-compose trap Coolify sprang today
- 65b0046 Give the worker the runner's address, and gate the sync on the real switch
- 906114f The worker keeps hosted agents' addresses honest
- 5b3002b A Coolify compose domain's port names the container, not the public side
- 8de9c46 Refresh the architecture map
- b3c44f8 Read the domain the proxy routes, not the one Coolify remembers
- 00c984c Give the workspace a say over its agents: budgets, and consent that shows the diff
- 677cb07 Refresh the architecture map
- 532d08c Give the client the run card, Later, and Catch Me Up
- 1a65937 Teach the server the three features that close the improvement pass
- 0e0513b Refresh the architecture map
- d22b71f Give the client the features its server was waiting to serve
- 4c21d7f Wire what the server already knew how to do, and shape the modules by audience
- 755744b Make the client truthful about failure, and give conversations real URLs
- b3bd7ac Close the cross-tenant and availability holes the audit found
- 58c6892 Refresh the architecture map
- 3f46ab1 See past the runner's preview twins
- caf348e Refresh the architecture map, and record eight traps from wiring both agent paths
- 7f36476 Apply the formatter that the lockfile now pins
- adfd313 Move the view rail into the top bar

**Decisions:** 0012-agents-may-dial-in
