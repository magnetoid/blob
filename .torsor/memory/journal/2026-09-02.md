...
**Summary:** 89 commit(s); 394 files changed, 19593 insertions(+), 3087 deletions(-); ran 17× check_drift, 3× check_dependencies, 3× check_staleness, 3× verify

Commits:
- f48ec3a Fix twenty-three defects an adversarial review found in today's work
- 5368213 Add /remind, which is a scheduled message to yourself
- 814a22f Show the build card for what it knows
- b8021db Make the build stamp survive a host with no repository
- 29611cc Walk the sidebar with ⌥↑ and ⌥↓, and stop a build stamp from blocking a deploy
- 8578d58 Tell people which build they are reading
- c029411 Add the slash commands a Slack user already types
- 43410e1 Close twenty holes a second defect hunt found
- f9683ee Let a scheduled message come back
- 33289f7 Delete the file before the row that knows where it is
- 0529d75 Keep a message that arrives while the channel is still loading
- 1a666b2 Draw the unread divider on the first row, where it went missing
- 60983cc Stop one live message marking a backlog you never saw as read
- 39cba7b Read a client's time the same way on all four routes that take one
- e354277 Stop the mention badge counting a message you have already read
- 10374bd Stop one failed page from killing scrollback for the session
- b4fa288 Stop one person's saved preferences from silencing everyone else's
- 57f3516 Actually send a scheduled message, rather than only storing it
- 8d90244 Merge slack-parity-pass: a design pass, then two dozen real defects
- 1d14d9a Say what @here actually does, which is what @channel does
