...
**Summary:** 20 commit(s); 454 files changed, 32540 insertions(+), 4068 deletions(-); ran 21× check_drift, 1× check_dependencies, 1× check_staleness, 1× verify

Commits:
- df388b5 Merge pull request #3 from magnetoid/feat/shell_dashboard_hardening
- 7b8c1c8 Refine the chat shell and harden live dashboard updates.
- fe69eeb Stop trusting a header the caller writes, and four more places a member reached too far
- 55bafe0 Let your own assistant read this workspace, and say when a file cannot be uploaded
- a8e0535 Say when a notification cannot reach anybody, and stop reporting failures as delivery
- bae5242 Images: a small copy for the channel, the original when you ask for it
- 9d3b153 Activity: everybody who named you, and everybody who reacted to what you wrote
- a7e2da8 A channel frame stops carrying your place in the channel; search learns time order
- 47cac0b Summaries a model writes and you can check; questions that come back to whoever asked
- 1533f4a Record the trap that took the host down: two Coolify builds at once
- ab21354 Work channels: a channel spun from a thread, with plan, changes and preview tabs
- cc35443 Put a Content-Security-Policy on every response
- e4df5e8 Let a member connect their own agent, and let an agent remember
- 097c115 Let agents answer each other, and let a person answer an agent
- 645d250 Give an agent an owner, so a personal one is personal
- a855675 Let a thread be left, and a channel be reopened
- 40461d5 Read the bits that were only ever written
- 3a3efcf Rebuild the README, and make three of its claims true
- 1c4636d Let the map catch up with the guide's parity test
- f9305d0 Answer "how does this work?" inside the app

**Decisions:** 0013-agent-chains-carry-human-authority, 0014-work-channels-and-sandboxed-artifacts, 0015-summaries-cite-and-nudges-stay-private, 0016-an-assistant-token-is-a-person
